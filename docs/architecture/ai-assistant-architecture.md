...
bounded text streaming path for configured providers, provider-native and
`<think>` fallback reasoning deltas, generated/refined thread titles, and
...
event contract (`system`, `textDelta`, `reasoningDelta`, `threadTitleUpdated`,
