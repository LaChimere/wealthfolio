...
updated_at: "2026-06-21T05:39:12+08:00"
<!-- prettier-ignore-end -->
...
- Turn 657: Tightened custom-provider JSON numeric-string locale parity. JSON
  row extraction now ignores configured locale for string numbers like Rust, so
  `"4,832"` with `de-DE` resolves through auto-detection as `4832` instead of a
  decimal comma value. Verified with focused custom-provider tests.
