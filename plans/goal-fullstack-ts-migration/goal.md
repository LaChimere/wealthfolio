...
- Real Connect token lifecycle, cloud HTTP clients, broker sync orchestration,
  local sync repositories, broker sync profile persistence, subscription
  entitlement checks, event production, and device-sync enrollment/engine
  behavior remain active follow-ups. reason=the standalone TS backend now wires
  disabled Connect feature-flag behavior and empty local Connect list routes,
