...
updated_at: "2026-07-01T00:39:21+09:00"
<!-- prettier-ignore-end -->
...
- Turn 1045: Corrected unknown Connect reconcile handling to match Rust. Unknown
  ...
  repository check passed.
- Turn 1046: Strengthened unknown Connect reconcile pull-tail evidence. The
  ...
  check passed.
- Turn 1047: Ran dual-model final reconcile parity review with Claude Opus 4.8
  xhigh and GPT-5.5 xhigh. Both reviews found no actionable Rust parity, branch
  ordering, pending-outbox, cursor/lock-state, test-validity, or overbroad
  unknown-action handling issues.
- Turn 1048: Added add-on SDK permission metadata evidence for
  `events.market.onSyncError`. The SDK package now has a Bun test proving the
  event listener is exposed by `PERMISSION_CATEGORIES`, recognized by
  `isPermissionRequired`, and classified as low risk. Focused SDK test,
  addon-sdk type-check, and full repository check passed.
- Turn 1049: Strengthened unknown Connect reconcile pending-push evidence. The
  runtime trigger-cycle push smoke now proves unknown but well-formed reconcile
  actions with pending outbox rows take the normal push path, encrypt and submit
  the outbox event, and mark it sent. Trigger-cycle runtime tests and full
  repository check passed.
- Turn 1050: Fixed the add-on SDK permission test execution gap found by review.
  The SDK package now has a `test` script, the root has `test:addon-sdk`,
  `test:all` includes it, and PR/CI full checks run the add-on SDK tests so the
...
- Turn 1051: Fixed the backend-contracts test execution gap found during the
  same review pass. Root scripts now expose `test:backend-contracts`, `test:all`
...
- Turn 1053: Extended the backend-contracts add-on host canary to include
  `market:sync-error`. Contract tests now guard that the backend event producer,
  web EventSource adapter, and Electron adapter all keep the new market sync
  error event visible alongside sync-complete. Backend-contracts tests and full
  repository check passed.
- Turn 1054: Ran the full CI script after package test wiring and fixed two
  blockers it exposed: Rust config tests now live after all non-test items to
...
- Turn 1056: Added runtime Portfolio account-scope route evidence. The
  ...
  and full repository check passed.
- Turn 1057: Strengthened contribution-limit runtime route evidence. The
...
- Turn 1058: Completed runtime Portfolio account-scope query route coverage. The
...
- Turn 1059: Addressed review feedback on the Portfolio account-scope runtime
  smoke. The test now seeds distinct included and excluded account holdings and
...
- Turn 1061: Ran dual-model Portfolio scope/deposit evidence review with Claude
  Opus 4.8 xhigh and GPT-5.5 xhigh. Both reviews found no actionable remaining
  false-positive, DTO-shape, fixture, expected-total, exclusion, contribution
  deposit, or scope-creep issues.
- Turn 1062: Added runtime general-purpose custom-provider sync evidence. The
  market-data sync route now proves a `CUSTOM_SCRAPER` asset without an explicit
  ...
  full repository check passed.
- Turn 1063: Added runtime general-purpose custom-provider historical backfill
  evidence. The market-data sync route now proves a `CUSTOM_SCRAPER` asset
  without `custom_provider_code` can select a persisted historical custom
...
- Turn 1064: Added runtime AI chat tool execution evidence. The standalone TS
  ...
  passed.
- Turn 1065: Ran dual-model provider/AI runtime evidence review and addressed
  the actionable finding. The AI stream smoke now asserts the exact normalized
  tool allowlist (`get_accounts`, `get_cash_balances`) and excludes mutation/CSV
  tools, while the general custom-provider runtime evidence had no actionable
  issues. Focused provider/AI runtime tests and full repository check passed.
- Turn 1066: Tightened runtime AI tool-result assertions after the allowlist
  review fix. The stream smoke now verifies both `get_accounts` and
  `get_cash_balances` execute successfully, the cash tool returns base-currency
  data, and the follow-up provider request carries the cash tool result. Focused
  AI runtime test and full repository check passed.
- Turn 1067: Added runtime AI `get_goals` tool execution evidence. The
...
- Turn 1068: Added runtime AI `get_health_status` tool execution evidence. The
  ...
  provider request. Focused runtime AI tests and full repository check passed.
- Turn 1069: Fixed the provider/AI review regression. The cash-balance fixture
  is now seeded in the AI tool stream runtime DB instead of an unrelated Connect
  session test, and the AI stream assertion verifies `get_cash_balances` returns
  the seeded USD cash total. Focused Connect/AI runtime tests and full
  repository check passed.
- Turn 1070: Added runtime AI `search_activities` tool execution evidence. The
  standalone TS runtime now proves `/api/v1/ai/chat/stream` can execute
  `search_activities` against persisted activity data, expose the normalized
  activity-tool allowlist including draft/mutation helpers, stream the tool
  result, and pass it into the follow-up provider request. Focused runtime AI
  tests and full repository check passed.
- Turn 1071: Ran dual-model AI toolset runtime review with Claude Opus 4.8 xhigh
  and GPT-5.5 xhigh. Both reviews found no actionable false-positive, allowlist,
  global-fetch, fixture isolation, tool-result, provider follow-up, or scope
  issues for the runtime tool stream evidence.
- Turn 1072: Added runtime AI `get_income` tool execution evidence. The
  standalone TS runtime now proves `/api/v1/ai/chat/stream` can execute
  `get_income` against persisted dividend activity data, stream YTD income
  totals and top-asset payloads, and pass the parsed tool result into the
  follow-up provider request. Focused runtime AI tests and full repository check
  passed.
- Turn 1073: Addressed AI toolset review feedback for income. The runtime
...
- Turn 1077: Added runtime AI `record_activity` draft tool execution evidence.
  The standalone TS runtime now proves `/api/v1/ai/chat/stream` can execute
  `record_activity` against persisted account data, expose only the allowed
  draft tool, stream a Rust-shaped valid cash-flow draft, pass the parsed draft
...
- Turn 1078: Added runtime AI `record_activities` batch draft tool execution
  evidence. The standalone TS runtime now proves `/api/v1/ai/chat/stream` can
  execute `record_activities` against persisted account data, expose only the
  allowed batch draft tool, stream Rust-shaped valid cash-flow batch drafts,
  pass the parsed batch result into the follow-up provider request, and preserve
  the tool's no-persistence behavior. Focused runtime AI tests and full
  repository check passed.
- Turn 1079: Added runtime AI `import_csv` tool execution evidence. The
  standalone TS runtime now proves `/api/v1/ai/chat/stream` can execute
  `import_csv` through the real CSV parse seam, expose only the allowed CSV
  tool, stream detected headers/sample rows/applied mappings, pass the parsed
  mapping result into the follow-up provider request, and preserve the tool's
  no-import/no-persistence behavior. Focused runtime AI tests and full
  repository check passed.
- Turn 1080: Ran dual-model AI draft/import tool review with Claude Opus 4.8
  xhigh and GPT-5.5 xhigh. Both reviews found no actionable issues in tool
  execution proof, provider follow-up assertions, fixture validity, cleanup,
  allowlist filtering, no-persistence assertions, or plan/todo consistency.
- Turn 1081: Added runtime AI `get_holdings` tool execution evidence. The
  standalone TS runtime now proves `/api/v1/ai/chat/stream` can execute
  `get_holdings` against persisted holdings snapshot, asset, and quote rows,
  expose only the allowed holdings tool, stream Rust-shaped holdings payloads,
  and pass parsed holdings into the follow-up provider request. Focused runtime
  AI tests and full repository check passed.
- Turn 1082: Addressed dual-model holdings review feedback. The runtime
  `get_holdings` stream smoke now asserts quote-derived `marketValueBase`,
  `costBasisBase`, and `unrealizedGainPct`, making the seeded quote row
  load-bearing in the AI tool evidence. Focused runtime AI tests and full
  repository check passed.
...
- Turn 1084: Hardened local add-on archive install/staging paths. The TS add-on
  ...
  and full repository check passed.
- Turn 1085: Addressed add-on hardening review feedback. The filesystem add-on
  ID guard now rejects bare dot IDs (`.` and `..`) before install/staging/store
  download work, closing the add-ons-root deletion and store endpoint
  normalization gap found by dual-model security review. Focused add-on tests
  and full repository check passed.
...
- Turn 1087: Hardened add-on query-cache access. Query cache helpers now require
  ...
  repository check passed.
- Turn 1088: Ran dual-model add-on query-cache hardening review with Claude Opus
  4.8 xhigh and GPT-5.5 xhigh. Both reviews found no actionable issues in
...
- Turn 1090: Addressed device registration rollback review feedback. Device
  ...
  unit/runtime tests and full repository check passed.
- Turn 1091: Ran dual-model device registration concurrency review with Claude
  Opus 4.8 xhigh and GPT-5.5 xhigh. Both reviews confirmed the serialized
  persistence queue fixes the prior rollback clobbering issue and found no new
  actionable issues.
- Turn 1092: Extended device-sync secret serialization to pairing-flow
  cancellation cleanup. Local cancel cleanup now shares the registration
  persistence queue and conditionally skips clearing secrets if a concurrent
  successful registration has moved `sync_identity`/`sync_device_id` to a new
  device. Focused pairing/concurrency tests and full repository check passed.
- Turn 1093: Addressed pairing-flow cancellation review feedback. Pairing flow
...
- Turn 1094: Extended flow-device binding to approved/polled pairing-flow
  transitions. `approvePairingOverwrite` and `getPairingFlowState` now use the
  original flow device for remote snapshot checks instead of the current
  `sync_identity`, preserving old flow cancellability after a later device
  registration. Focused pairing/concurrency tests and full repository check
  passed.
- Turn 1095: Ran dual-model pairing flow-device review with Claude Opus 4.8
  xhigh and GPT-5.5 xhigh. Both reviews confirmed approve/poll now use the
  original flow device after later registration, old flows remain cancellable,
  and no new actionable issues were found.
- Turn 1096: Fixed market-data search provider identity parity for Finnhub
  fallback results. TS search results from Finnhub now report
  `dataSource: "FINNHUB"` like the Rust Finnhub provider instead of inheriting
  Yahoo's data source label. Focused market-data/runtime tests and full
  repository check passed.
- Turn 1097: Addressed Finnhub source review feedback. Unrelated Yahoo quote
  history/existing-asset expectations were restored to `YAHOO`, and the missed
  non-MIC Finnhub fallback assertion now expects `FINNHUB`. Focused
  market-data/runtime tests and full repository check passed.
- Turn 1098: Strengthened Connect enable device-id persistence evidence.
...
- Turn 1099: Addressed recent parity review feedback. The Yahoo-secondary search
  ...
  tests and full repository check passed.
- Turn 1100: Strengthened runtime background engine evidence. The
...
- Turn 1101: Ran dual-model review over recent market-data/device-sync runtime
  ...
  flakiness.
- Turn 1102: Made Connect enable compatibility device-id persistence
  Rust-compatible and non-fatal. `/connect/device/enable` now treats
  `sync_identity` as canonical and logs, rather than fails, if the legacy
  `sync_device_id` compatibility secret cannot be written after enrollment
  succeeds. Focused Connect/runtime tests and full repository check passed.
- Turn 1103: Ran dual-model review for the Connect legacy device-id best-effort
  slice with Claude Opus 4.8 xhigh and GPT-5.5 xhigh. Both reviews found no
  actionable correctness, security, parity, or consumer-compatibility issues.
- Turn 1104: Hardened unsupported single-file add-on install adapters. The web
  adapter no longer echoes addon source content in the unsupported
  `installAddonFile` error, and web/Electron tests now cover that no unsupported
  path leaks file contents or invokes Electron. Focused frontend adapter tests
  and full repository check passed.
- Turn 1105: Ran dual-model security review for unsupported add-on file install
  hardening with Claude Opus 4.8 xhigh and GPT-5.5 xhigh. Both reviews confirmed
  the web adapter no longer leaks file contents, Electron remains non-invoked,
  tests are meaningful, and status docs are accurate.
- Turn 1106: Added provider-backed symbol resolution for direct activity
  creates. Direct `/api/v1/activities` BUY/SELL-style creates can now resolve
  symbol-only asset inputs through the existing market-data search seam before
  staging a new activity-created asset, while keeping existing asset matches and
  explicit exchange identities unchanged. Focused activity unit/runtime tests
  and full repository check passed.
- Turn 1107: Addressed direct activity provider-resolution review feedback. The
  resolver now preserves explicit metadata from legacy `symbol: { ... }` asset
  objects when converting provider-resolved direct creates into canonical
  `asset` inputs. Focused activity unit/runtime tests and full repository check
  passed.
- Turn 1108: Extended provider-backed direct activity resolution to updates.
  Direct `/api/v1/activities` updates can now resolve symbol-only asset inputs
  through the market-data search seam before staging a new activity-created
  asset, matching the direct create behavior while preserving existing update
  paths. Focused activity unit/runtime tests and full repository check passed.
- Turn 1109: Addressed direct activity update review feedback. Partial transfer
  updates now resolve provider-backed symbols using existing quantity/unit-price
  values when those numeric fields are omitted, so valid symbol-only transfer
  patches do not skip provider resolution. Focused activity unit/runtime tests
  and full repository check passed.
- Turn 1110: Addressed provider-resolution update race feedback. Activity
