...
- [ ] PR 5+: Domain vertical slices.
  ...
  - Evidence: settings, accounts, contribution limits, taxonomy, custom
    ...
    deletion, and bounded manual snapshot saves now have TS runtime parity,
    while provider-backed symbol performance history and holdings import
    provider-backed symbol search are deferred to portfolio/market-data parity
    slices; snapshot import writes and portfolio recalculation side effects are
    ...
    symbol-only activity asset creation, import execution, provider-backed asset
    ...
    slices; AI chat persistence and tool-result mutation now have TS runtime
    parity, while provider streaming, tool execution, tag persistence, and
...
- Outcome: PR 1 contract foundation, PR 2 guarded TS backend runtime skeleton,
  ...
  snapshot deletion runtime, and bounded manual snapshot save runtime
...
- Follow-ups: continue other low-risk domain slices; broader health
  ...
  history moves with market-data/provider parity slices; snapshot import
  writes/provider-backed symbol search and portfolio recalculation side effects
  move with holdings/portfolio parity slices; add-on filesystem extraction,
  runtime loading, store HTTP, staging I/O, and update behavior move with add-on
  runtime parity slices; symbol-only activity asset creation, quote fallback
