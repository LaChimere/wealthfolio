...
- `pr5-runtime-device-sync-device-cloud-read-route-smoke`: verification passed.
  Runtime-backed standalone device reads now expect `/api/v1/sync/devices` and
...
- `pr5-runtime-connect-bootstrap-already-complete-route-smoke`: verification
...
- `pr5-runtime-connect-bootstrap-newer-schema-route-smoke`: verification passed.
...
- `pr5-runtime-connect-route-review-2`: dual-model review passed. Claude Opus
  4.8 xhigh and GPT-5.5 xhigh reviewed trigger-cycle PULL_TAIL, bootstrap
  no-remote, generate export-gate, and enable READY route smokes and found no
  actionable issues.
- `pr5-runtime-connect-enable-stale-route-smoke`: verification passed.
  Runtime-backed Connect enable now expects `/api/v1/connect/device/enable` to
...
- `pr5-runtime-connect-enable-route-review`: dual-model review passed. Claude
...
- `pr5-runtime-connect-pull-event-validation`: verification passed. Pull-tail
...
- `pr5-runtime-connect-goal-plan-replay-route-smoke`: verification passed.
  ...
  metadata/applied event tracking.
- `pr5-runtime-connect-goals-allocation-replay-route-smoke`: verification
  passed. Runtime-backed trigger-cycle now decrypts and applies remote
...
- `pr5-runtime-connect-activity-import-profile-replay-route-smoke`: verification
  ...
  tracking.
- `pr5-runtime-connect-import-run-replay-route-smoke`: verification passed.
...
- `pr5-runtime-connect-ai-thread-replay-route-smoke`: verification passed.
  ...
  metadata/applied-event tracking.
- `pr5-runtime-connect-ai-message-replay-route-smoke`: verification passed.
...
- `pr5-runtime-connect-snapshot-replay-route-smoke`: verification passed.
  Runtime-backed trigger-cycle now decrypts and applies remote snapshot
  create/update/delete events into holdings_snapshots, clears stale
...
- `pr5-runtime-device-sync-pairing-transfer-snapshot-upload`: verification
  passed. Standalone complete-with-transfer now mirrors Rust by always running
  the migrated Connect sync cycle, preserving pending-outbox/dead-letter safety
  gates, requiring generate-snapshot/upload to return `uploaded`, and then
  minting a fresh post-snapshot token before cloud pairing approve/complete for
  any local DB-backed source device. Skipped/cancelled/malformed snapshot
  outcomes block transfer, and runtime coverage proves already-bootstrapped,
...
- `pr5-runtime-market-data-provider-skip-wording`: verification passed. Unknown
  preferred market-data providers now report
...
- `pr5-alpha-vantage-option-route-resolve`: verification passed. The runtime
  ...
  backend type-check passed.
- `pr5-finnhub-route-provider-smoke`: verification passed. The runtime
  `/api/v1/market-data/sync` route now has SQLite-backed coverage for a
...
- `pr5-openfigi-fallback-route-provider-smoke`: verification passed. The runtime
  `/api/v1/market-data/sync` route now has SQLite-backed coverage for an
  OpenFIGI-preferred asset, proving quote sync avoids profile-only OpenFIGI
...
- `pr5-finnhub-route-resolve-smoke`: verification passed. The runtime
  `/api/v1/market-data/resolve-currency` route now has coverage for Finnhub
  equity, FX, and crypto quote summaries, proving backend-stored API key auth,
...
- `pr5-metal-price-route-resolve-smoke`: verification passed. The runtime
  ...
  full repository check passed.
- `pr5-us-treasury-route-resolve-smoke`: verification passed. The runtime
  `/api/v1/market-data/resolve-currency` route now has coverage for a US
  Treasury calculated bond quote summary, proving existing bond metadata is read
...
- `pr5-market-data-history-route-smoke`: verification passed. The runtime
  ...
  full repository check passed.
- `pr5-health-full-classification-fix-runtime`: verification passed. The runtime
  `/api/v1/health/fix` route now has focused coverage for
  `migrate_legacy_classifications`, proving all legacy-classified assets are
  migrated, legacy metadata is cleaned on each migrated asset, and
  `asset_taxonomy_assignment` sync-outbox rows are queued. Full runtime suite
  and full repository check passed.
- `pr5-health-legacy-classification-issue-runtime`: verification passed. The
  runtime `/api/v1/health/check` route now has focused coverage for
  SQLite-backed legacy classification issue generation, proving affected-item
...
- `pr5-health-dismissal-route-runtime`: verification passed. The runtime
  ...
  check passed.
- `pr5-health-config-route-runtime`: verification passed. The runtime
  `/api/v1/health/config` GET/PUT routes now have focused coverage proving
...
- `pr5-market-sync-failure-event-runtime`: verification passed. The runtime
  `/api/v1/market-data/sync` route now has focused coverage for provider
  failures, proving HTTP 204 is preserved, `market:sync-complete` publishes the
...
- `pr5-frontend-electron-market-sync-payload`: verification passed. The frontend
  Electron adapter test now proves `market:sync-complete` preload events
...
- `pr5-global-market-sync-error-payload`: verification passed. The React global
  ...
  check passed.
- `pr5-addon-market-sync-error-event`: verification passed. Add-ons can now
  declare and use `events.market.onSyncError`; SDK/permission metadata and the
  host bridge expose the event, backend static permission detection recognizes
  `ctx.api.events.market.onSyncError`, and web/Electron adapter tests prove
...
- `pr5-connect-unknown-reconcile-push-pull`: verification passed. Unknown but
  well-formed reconcile actions now proceed through the normal push/pull flow
...
- `pr5-runtime-ai-health-status-tool`: verification passed. The standalone TS
  runtime now proves `/api/v1/ai/chat/stream` can execute the injected
  health-status tool against the composed runtime health service, stream the
  not-computed health payload, and pass it into the follow-up provider request.
  Focused runtime AI tests and full repository check passed.
- `pr5-runtime-ai-cash-fixture-review-fix`: verification passed. The
  cash-balance fixture is now seeded in the AI tool stream runtime DB instead of
...
- `pr5-runtime-ai-income-tool`: verification passed. The standalone TS runtime
  now proves `/api/v1/ai/chat/stream` can execute `get_income` against persisted
...
- `pr5-runtime-ai-asset-allocation-tool`: verification passed. The standalone TS
  ...
  runtime AI tests and full repository check passed.
- `pr5-runtime-ai-record-activity-tool`: verification passed. The standalone TS
  runtime now proves `/api/v1/ai/chat/stream` can execute `record_activity`
  against persisted account data, expose only the allowed draft tool, stream a
  Rust-shaped valid cash-flow draft, pass the parsed draft into the follow-up
  provider request, and preserve the tool's no-persistence behavior. Focused
  runtime AI tests and full repository check passed.
- `pr5-runtime-ai-record-activities-tool`: verification passed. The standalone
  TS runtime now proves `/api/v1/ai/chat/stream` can execute `record_activities`
  against persisted account data, expose only the allowed batch draft tool,
...
- `pr5-connect-enable-device-id-secret-evidence`: verification passed. Existing
  ...
  tests and full repository check passed.
- `pr5-market-data-yahoo-secondary-source-review-fix`: verification passed.
  Review found the Yahoo-secondary search fallback assertion was accidentally
...
- `pr5-connect-enable-legacy-device-id-best-effort`: verification passed.
  `/connect/device/enable` now treats `sync_identity` as canonical and logs,
