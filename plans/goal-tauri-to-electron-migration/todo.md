...
| rust-service-bridge    | in_progress | Connect Electron to existing Rust-backed services, SQLite data dir behavior, and domain event streams.                        | Sidecar lifecycle, account/settings, temporary secrets, sync crypto, portfolio, snapshots/imports, activities, exchange rates, market-data providers, contribution limits, assets, quotes/imports/sync, taxonomies, Health Center, Net Worth, alternative assets, add-ons, AI providers/thread metadata/streaming, goals, and retirement proxy tests passed. |
