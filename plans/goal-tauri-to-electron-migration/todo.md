...
| rust-service-bridge    | in_progress | Connect Electron to existing Rust-backed services, SQLite data dir behavior, and domain event streams.                        | Sidecar profile/lifecycle done; account list/create/update/delete IPC→main→sidecar proxy, redaction, validation, and tests passed. |
