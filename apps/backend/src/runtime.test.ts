...
describe("TS backend runtime composition", () => {
  ...
  test("wires runtime health retry sync fix to provider quote persistence", async () => {
    const appDataDir = mkdtempSync(path.join(tmpdir(), "wealthfolio-runtime-health-retry-fix-"));
    const chartSymbols: string[] = [];
    const runtime = createSqliteBackedBackendServices({
      appDataDir,
      marketDataFetch: ((input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url === "https://fc.yahoo.com") {
          return Promise.resolve(
            new Response("", { headers: { "set-cookie": "B=health-retry; Path=/; Secure" } }),
          );
        }
        if (url === "https://query1.finance.yahoo.com/v1/test/getcrumb") {
          expect((init?.headers as Record<string, string>).Cookie).toBe("B=health-retry");
          return Promise.resolve(new Response("health-retry-crumb"));
        }
        if (url.startsWith("https://query1.finance.yahoo.com/v8/finance/chart/RETRY?")) {
          expect((init?.headers as Record<string, string>).Cookie).toBe("B=health-retry");
          const parsed = new URL(url);
          expect(parsed.searchParams.get("crumb")).toBe("health-retry-crumb");
          chartSymbols.push("RETRY");
          return Promise.resolve(
            Response.json({
              chart: {
                result: [
    ...
    try {
      const seedDb = openSqliteDatabase(runtime.dbPath);
      try {
        seedRuntimeAsset(seedDb, "health-retry-asset");
        seedDb
          .prepare(
            "UPDATE assets SET display_code = ?, instrument_symbol = ?, instrument_exchange_mic = ? WHERE id = ?",
          )
          .run("RETRY", "RETRY", "XNAS", "health-retry-asset");
      } finally {
        seedDb.close();
      }
      ...
      const response = await fetch(`${server.baseUrl}/api/v1/health/fix`, {
        ...
        body: JSON.stringify({
          id: "retry_sync",
          label: "Retry Sync",
          payload: ["health-retry-asset"],
        }),
      });
      expect(response.status).toBe(200);
      expect(chartSymbols).toEqual(["RETRY"]);
        ...
        expect(
          verifyDb
            .query<
              { asset_id: string; source: string; close: string; currency: string },
              []
            >("SELECT asset_id, source, close, currency FROM quotes WHERE asset_id = 'health-retry-asset'")
            .get(),
        ).toEqual({
          asset_id: "health-retry-asset",
          source: "YAHOO",
          close: "34.56",
          currency: "USD",
        });
  ...
  test("streams runtime AI chat tool calls through injected portfolio tools", async () => {
    const appDataDir = mkdtempSync(path.join(tmpdir(), "wealthfolio-runtime-ai-tool-stream-"));
    const runtime = createSqliteBackedBackendServices({
      appDataDir,
      repositoryRoot,
      secretKey: config.secretKey,
    ...
    try {
        ...
        seedDb
          .prepare(
            `
              ...
              VALUES (
                'ai-tool-account', 'Runtime AI Tool Account', 'SECURITIES',
                NULL, 'USD', 0, 1, 0, 'HOLDINGS'
              )
            `,
          )
          .run();
      ...
      globalThis.fetch = (async (input, init) => {
        ...
        return new Response(
          '{"message":{"content":"Accounts loaded."},"done":false}\n{"done":true}\n',
          { headers: { "content-type": "application/x-ndjson" } },
        );
      }) as typeof fetch;
      ...
      const streamResponse = await fetch(`${server.baseUrl}/api/v1/ai/chat/stream`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ content: "Show accounts" }),
      });
  ...
  test("streams runtime AI chat goal tool results through provider follow-up", async () => {
    const appDataDir = mkdtempSync(path.join(tmpdir(), "wealthfolio-runtime-ai-goal-tool-"));
    const runtime = createSqliteBackedBackendServices({
      appDataDir,
      repositoryRoot,
      secretKey: config.secretKey,
    ...
    try {
      ...
      if (!goalService) {
        throw new Error("Runtime AI goal tool test requires goal service");
      }
      await goalService.createGoal({
        goalType: "custom_save_up",
        title: "Runtime AI Goal",
        targetAmount: 5_000,
        currency: "USD",
      });
      ...
      globalThis.fetch = (async (input, init) => {
        ...
        if (providerBodies.length === 1) {
          return new Response(
            '{"message":{"tool_calls":[{"id":"runtime-goals-call","function":{"name":"get_goals","arguments":{}}}]},"done":false}\n{"done":true}\n',
            { headers: { "content-type": "application/x-ndjson" } },
          );
        }
        return new Response(
          '{"message":{"content":"Goals loaded."},"done":false}\n{"done":true}\n',
          { headers: { "content-type": "application/x-ndjson" } },
        );
      }) as typeof fetch;
      ...
      const streamResponse = await fetch(`${server.baseUrl}/api/v1/ai/chat/stream`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ content: "Show goals" }),
      });
      expect(streamResponse.status).toBe(200);
      const events = (await streamResponse.text())
        .trim()
      ...
      expect(JSON.stringify(events)).toContain("Runtime AI Goal");
      expect((providerBodies[0]?.tools ?? []) as Array<{ function?: { name?: string } }>).toEqual([
        expect.objectContaining({ function: expect.objectContaining({ name: "get_goals" }) }),
      ]);
      expect(JSON.stringify(providerBodies[1]?.messages)).toContain("Runtime AI Goal");
    } finally {
      globalThis.fetch = originalFetch;
      server.stop();
      await runtime.close();
  ...
  test("streams runtime AI chat health tool results through provider follow-up", async () => {
    const appDataDir = mkdtempSync(path.join(tmpdir(), "wealthfolio-runtime-ai-health-tool-"));
    const runtime = createSqliteBackedBackendServices({
      appDataDir,
      repositoryRoot,
      secretKey: config.secretKey,
    ...
    try {
      const updateResponse = await fetch(`${server.baseUrl}/api/v1/ai/providers/settings`, {
        ...
        body: JSON.stringify({
          ...
          enabled: true,
          selectedModel: "qwen3.5:9b",
          toolsAllowlist: ["get_health_status"],
        }),
      });
      expect(updateResponse.status).toBe(200);
      ...
      globalThis.fetch = (async (input, init) => {
        ...
        if (providerBodies.length === 1) {
          return new Response(
            '{"message":{"tool_calls":[{"id":"runtime-health-call","function":{"name":"get_health_status","arguments":{}}}]},"done":false}\n{"done":true}\n',
            { headers: { "content-type": "application/x-ndjson" } },
          );
        }
        return new Response(
          '{"message":{"content":"Health checked."},"done":false}\n{"done":true}\n',
          { headers: { "content-type": "application/x-ndjson" } },
        );
      }) as typeof fetch;
      ...
      const streamResponse = await fetch(`${server.baseUrl}/api/v1/ai/chat/stream`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ content: "Show health" }),
      });
      expect(streamResponse.status).toBe(200);
      const events = (await streamResponse.text())
        .trim()
      ...
      expect(JSON.stringify(events)).toContain("NOT_COMPUTED");
      expect(JSON.stringify(providerBodies[1]?.messages)).toContain("NOT_COMPUTED");
    } finally {
      globalThis.fetch = originalFetch;
      server.stop();
      await runtime.close();
  ...
  test("streams runtime AI chat activity search tool results through provider follow-up", async () => {
    const appDataDir = mkdtempSync(path.join(tmpdir(), "wealthfolio-runtime-ai-activity-tool-"));
    const runtime = createSqliteBackedBackendServices({
      appDataDir,
      repositoryRoot,
      secretKey: config.secretKey,
    ...
    try {
      ...
      const updateResponse = await fetch(`${server.baseUrl}/api/v1/ai/providers/settings`, {
        ...
        body: JSON.stringify({
          ...
          enabled: true,
          selectedModel: "qwen3.5:9b",
          toolsAllowlist: ["search_activities"],
        }),
      });
      expect(updateResponse.status).toBe(200);
      ...
      globalThis.fetch = (async (input, init) => {
        ...
        if (providerBodies.length === 1) {
          return new Response(
            '{"message":{"tool_calls":[{"id":"runtime-activity-search-call","function":{"name":"search_activities","arguments":{"accountId":"Transaction Account","activityType":"BUY","symbol":"TX"}}}]},"done":false}\n{"done":true}\n',
            { headers: { "content-type": "application/x-ndjson" } },
          );
        }
        return new Response(
          '{"message":{"content":"Activities loaded."},"done":false}\n{"done":true}\n',
          { headers: { "content-type": "application/x-ndjson" } },
        );
      }) as typeof fetch;
      ...
      const streamResponse = await fetch(`${server.baseUrl}/api/v1/ai/chat/stream`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ content: "Find TX buys" }),
      });
      expect(streamResponse.status).toBe(200);
      const events = (await streamResponse.text())
        .trim()
      ...
      expect(JSON.stringify(events)).toContain("tx-buy");
      expect(JSON.stringify(events)).toContain("Transaction Account");
      expect(JSON.stringify(providerBodies[1]?.messages)).toContain("tx-buy");
    } finally {
      globalThis.fetch = originalFetch;
      server.stop();
      await runtime.close();
  ...
  test("streams runtime AI chat asset allocation tool results through provider follow-up", async () => {
    const appDataDir = mkdtempSync(path.join(tmpdir(), "wealthfolio-runtime-ai-allocation-tool-"));
    const runtime = createSqliteBackedBackendServices({
      appDataDir,
      repositoryRoot,
      secretKey: config.secretKey,
    ...
    try {
        ...
        seedDb
          .prepare(
            `
              ...
              VALUES ('ai-allocation-account', 'AI Allocation', 'CASH', NULL, 'USD', 0, 1, 0, 'HOLDINGS')
            `,
          )
          .run();
      } finally {
      ...
      if (!runtime.options.holdingsService) {
        throw new Error("AI allocation runtime test requires holdings service");
      }
      await runtime.options.holdingsService.saveManualHoldings({
        accountId: "ai-allocation-account",
      ...
      const updateResponse = await fetch(`${server.baseUrl}/api/v1/ai/providers/settings`, {
        ...
        body: JSON.stringify({
          ...
          enabled: true,
          selectedModel: "qwen3.5:9b",
          toolsAllowlist: ["get_asset_allocation"],
        }),
      });
      expect(updateResponse.status).toBe(200);
      ...
      globalThis.fetch = (async (input, init) => {
        ...
        if (providerBodies.length === 1) {
          return new Response(
            '{"message":{"tool_calls":[{"id":"runtime-allocation-call","function":{"name":"get_asset_allocation","arguments":{"accountId":"ai-allocation-account","groupBy":"class"}}}]},"done":false}\n{"done":true}\n',
            { headers: { "content-type": "application/x-ndjson" } },
          );
        }
        return new Response(
          '{"message":{"content":"Allocation loaded."},"done":false}\n{"done":true}\n',
          { headers: { "content-type": "application/x-ndjson" } },
        );
      }) as typeof fetch;
      ...
      const streamResponse = await fetch(`${server.baseUrl}/api/v1/ai/chat/stream`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ content: "Show allocation" }),
      });
      expect(streamResponse.status).toBe(200);
      const events = (await streamResponse.text())
        .trim()
      ...
      expect(exposedToolNames).toEqual(["get_asset_allocation"]);
      expect(JSON.stringify(events)).toContain('"categoryName":"Cash"');
      expect(JSON.stringify(events)).toContain('"totalValue":42');
      const allocationToolMessage = (
        (providerBodies[1]?.messages ?? []) as Array<{ role?: string; content?: string }>
      ).find((message) => message.role === "tool");
      expect(JSON.parse(String(allocationToolMessage?.content))).toMatchObject({
        totalValue: 42,
        currency: "USD",
        groupBy: "class",
        taxonomyId: "asset_classes",
        taxonomyName: "Asset Classes",
        allocations: [expect.objectContaining({ categoryId: "CASH", categoryName: "Cash" })],
      });
    } finally {
      globalThis.fetch = originalFetch;
      server.stop();
  ...
  test("streams runtime AI chat record activity draft results through provider follow-up", async () => {
    const appDataDir = mkdtempSync(path.join(tmpdir(), "wealthfolio-runtime-ai-record-tool-"));
    const runtime = createSqliteBackedBackendServices({
      appDataDir,
      repositoryRoot,
      secretKey: config.secretKey,
    ...
    try {
        ...
        seedDb
          .prepare(
            `
              ...
              VALUES ('ai-record-account', 'AI Draft Cash', 'CASH', NULL, 'USD', 0, 1, 0, 'TRANSACTIONS')
            `,
          )
          .run();
      } finally {
      ...
      const updateResponse = await fetch(`${server.baseUrl}/api/v1/ai/providers/settings`, {
        ...
        body: JSON.stringify({
          ...
          enabled: true,
          selectedModel: "qwen3.5:9b",
          toolsAllowlist: ["record_activity"],
        }),
      });
      expect(updateResponse.status).toBe(200);
      ...
      globalThis.fetch = (async (input, init) => {
        ...
        if (providerBodies.length === 1) {
          return new Response(
            '{"message":{"tool_calls":[{"id":"runtime-record-call","function":{"name":"record_activity","arguments":{"activityType":"DEPOSIT","activityDate":"2026-01-03","amount":25,"account":"AI Draft Cash","notes":"Runtime draft"}}}]},"done":false}\n{"done":true}\n',
            { headers: { "content-type": "application/x-ndjson" } },
          );
        }
        return new Response(
          '{"message":{"content":"Draft prepared."},"done":false}\n{"done":true}\n',
          { headers: { "content-type": "application/x-ndjson" } },
        );
      }) as typeof fetch;
      ...
      const streamResponse = await fetch(`${server.baseUrl}/api/v1/ai/chat/stream`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ content: "Draft a deposit" }),
      });
      expect(streamResponse.status).toBe(200);
      const events = (await streamResponse.text())
        .trim()
      ...
      expect(exposedToolNames).toEqual(["record_activity"]);
      expect(JSON.stringify(events)).toContain('"accountName":"AI Draft Cash"');
      expect(JSON.stringify(events)).toContain('"amount":25');
      const recordToolMessage = (
        (providerBodies[1]?.messages ?? []) as Array<{ role?: string; content?: string }>
      ).find((message) => message.role === "tool");
      expect(JSON.parse(String(recordToolMessage?.content))).toMatchObject({
        draft: {
          activityType: "DEPOSIT",
          activityDate: "2026-01-03",
          amount: 25,
          currency: "USD",
          accountId: "ai-record-account",
          accountName: "AI Draft Cash",
          notes: "Runtime draft",
        },
  ...
  test("streams runtime AI chat import CSV mapping results through provider follow-up", async () => {
    const appDataDir = mkdtempSync(path.join(tmpdir(), "wealthfolio-runtime-ai-import-csv-tool-"));
    const runtime = createSqliteBackedBackendServices({
      appDataDir,
      repositoryRoot,
      secretKey: config.secretKey,
    ...
    const originalFetch = globalThis.fetch;
    const providerBodies: Array<Record<string, unknown>> = [];
    const csvContent = "Date,Type,Symbol,Quantity,Price,Total\n2026-01-17,Buy,AAPL,2,100,200";
    ...
    try {
        ...
        seedDb
          .prepare(
            `
              ...
              VALUES ('ai-import-account', 'AI Import Account', 'SECURITIES', NULL, 'USD', 0, 1, 0, 'TRANSACTIONS')
            `,
          )
          .run();
      } finally {
      ...
      globalThis.fetch = (async (input, init) => {
          ...
          return new Response(
            `${JSON.stringify({
              message: {
                tool_calls: [
                  {
                    id: "runtime-import-csv-call",
                    function: {
                      name: "import_csv",
                      arguments: {
                        csvContent,
                        accountId: "ai-import-account",
                        fieldMappings: {
                          date: "Date",
                          activityType: "Type",
                          symbol: "Symbol",
        ...
        return new Response('{"message":{"content":"CSV mapped."},"done":false}\n{"done":true}\n', {
          headers: { "content-type": "application/x-ndjson" },
        });
      }) as typeof fetch;
      ...
      const streamResponse = await fetch(`${server.baseUrl}/api/v1/ai/chat/stream`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ content: "Import this CSV" }),
      });
      expect(streamResponse.status).toBe(200);
      const events = (await streamResponse.text())
        .trim()
      ...
      expect(exposedToolNames).toEqual(["import_csv"]);
      expect(JSON.stringify(events)).toContain('"detectedHeaders":["Date","Type","Symbol"');
      expect(JSON.stringify(events)).toContain('"totalRows":1');
      const importToolMessage = (
        (providerBodies[1]?.messages ?? []) as Array<{ role?: string; content?: string }>
      ).find((message) => message.role === "tool");
      expect(JSON.parse(String(importToolMessage?.content))).toMatchObject({
        accountId: "ai-import-account",
        detectedHeaders: ["Date", "Type", "Symbol", "Quantity", "Price", "Total"],
        sampleRows: [["2026-01-17", "Buy", "AAPL", "2", "100", "200"]],
        totalRows: 1,
  ...
  test("provider-resolves direct runtime activity updates to activity-created assets", async () => {
    const appDataDir = mkdtempSync(
      path.join(tmpdir(), "wealthfolio-runtime-activity-provider-update-"),
    );
    const marketCalls: string[] = [];
    const runtime = createSqliteBackedBackendServices({
      appDataDir,
    ...
    try {
      ...
      expect(updateResponse.status).toBe(200);
      const updated = (await updateResponse.json()) as { assetId?: string };
      expect(updated.assetId).toEqual(expect.any(String));
      expect(updated.assetId).not.toBe("tx-asset");
  ...
  test("wires runtime market-data sync route to general custom provider quotes", async () => {
    const appDataDir = mkdtempSync(
      path.join(tmpdir(), "wealthfolio-runtime-general-custom-sync-route-"),
    );
    const requests: string[] = [];
    const runtime = createSqliteBackedBackendServices({
      appDataDir,
      marketDataFetch: ((input: RequestInfo | URL) => {
        const url = String(input);
        requests.push(url);
        if (url !== "https://prices.example.test/general/FUND.TO") {
          throw new Error(`unexpected market data fetch: ${url}`);
        }
        return Promise.resolve(
          Response.json({
    ...
    try {
      ...
      try {
        seedDb
          ...
          .run(
            "runtime-general-custom-provider-id",
            "general-feed",
            "General Feed",
            "Runtime general feed",
            1,
            7,
            JSON.stringify({
              sources: [
                {
                  kind: "latest",
                  format: "json",
                  url: "https://prices.example.test/general/{SYMBOL}",
                  pricePath: "$.price",
                  datePath: "$.date",
                  currencyPath: "$.currency",
                },
        ...
        seedRuntimeAsset(seedDb, "runtime-general-custom-sync-asset");
        seedDb
          .prepare(
            "UPDATE assets SET display_code = ?, quote_ccy = ?, instrument_symbol = ?, instrument_exchange_mic = ?, provider_config = ? WHERE id = ?",
          )
          .run(
            ...
            JSON.stringify({
              preferred_provider: "CUSTOM_SCRAPER",
              overrides: {
                "CUSTOM:general-feed": { type: "equity_symbol", symbol: "FUND.TO" },
              },
            }),
            "runtime-general-custom-sync-asset",
          );
      } finally {
        seedDb.close();
      }
      ...
      const response = await fetch(`${server.baseUrl}/api/v1/market-data/sync`, {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          assetIds: ["runtime-general-custom-sync-asset"],
          refetchAll: false,
        }),
      });
      expect(response.status).toBe(204);
      expect(requests).toEqual(["https://prices.example.test/general/FUND.TO"]);
          ...
          verifyDb
            ...
            >(
              `
                ...
                FROM quotes q
                JOIN quote_sync_state s ON s.asset_id = q.asset_id
                WHERE q.asset_id = 'runtime-general-custom-sync-asset'
              `,
            )
            .get(),
        ).toEqual({
          id: "runtime-general-custom-sync-asset_2026-06-29_CUSTOM_SCRAPER:general-feed",
          source: "CUSTOM_SCRAPER:general-feed",
          close: "31.25",
          currency: "CAD",
          data_source: "CUSTOM_SCRAPER:general-feed",
        });
      } finally {
        verifyDb.close();
      }
  ...
  test("wires runtime quote resolution route to custom provider quote summaries", async () => {
    const appDataDir = mkdtempSync(path.join(tmpdir(), "wealthfolio-runtime-custom-resolve-"));
    const calls: string[] = [];
    const runtime = createSqliteBackedBackendServices({
      appDataDir,
      marketDataFetch: ((input: RequestInfo | URL) => {
        const url = String(input);
        calls.push(url);
        if (url !== "https://prices.example.test/FUND") {
          throw new Error(`unexpected market data fetch: ${url}`);
        }
        return Promise.resolve(
          Response.json({
    ...
    try {
      ...
      try {
        seedDb
          ...
          .run(
            "runtime-resolve-custom-provider-id",
            "my-feed",
            "My Feed",
            "Runtime custom quote source",
            1,
            1,
            JSON.stringify({
              sources: [
      ...
      const response = await fetch(
        `${server.baseUrl}/api/v1/market-data/resolve-currency?symbol=FUND&instrumentType=EQUITY&quoteCcy=cad&providerId=CUSTOM:my-feed`,
      );
      expect(response.status).toBe(200);
      await expect(response.json()).resolves.toEqual({
        currency: "CAD",
      ...
      expect(calls).toEqual(["https://prices.example.test/FUND"]);
    } finally {
      server.stop();
      await runtime.close();
    }
  ...
  test("wires runtime quote resolution route to US Treasury calculated bond quotes", async () => {
    const appDataDir = mkdtempSync(path.join(tmpdir(), "wealthfolio-runtime-treasury-resolve-"));
    const years: string[] = [];
    const treasuryXml = `<feed>
      <entry><content><m:properties>
        <d:NEW_DATE>2026-06-29T00:00:00</d:NEW_DATE>
    ...
    try {
      const seedDb = openSqliteDatabase(runtime.dbPath);
      try {
        seedRuntimeAsset(seedDb, "runtime-treasury-resolve-bond");
        seedDb
          .prepare(
            "UPDATE assets SET display_code = ?, quote_ccy = ?, instrument_type = ?, instrument_symbol = ?, provider_config = ?, metadata = ? WHERE id = ?",
          )
      ...
      const response = await fetch(
        `${server.baseUrl}/api/v1/market-data/resolve-currency?symbol=US912810TH12&instrumentType=BOND&quoteCcy=USD&providerId=US_TREASURY_CALC`,
      );
      expect(response.status).toBe(200);
