...
describe("TS local device sync service", () => {
  ...
  test("does not clear a concurrent successful registration while cancelling a pairing flow", async () => {
    const db = new Database(":memory:");
    db.exec(`
      CREATE TABLE sync_device_config (
        device_id TEXT PRIMARY KEY NOT NULL,
    ...
    const service = createLocalDeviceSyncService({
      ...
      fetch: async (input, init) => {
        ...
        if (url.endsWith("/pairings/pairing-1/cancel")) {
          await new Promise((resolve) => setTimeout(resolve, 20));
        }
        return Response.json({ success: true, key_version: 2 });
      },
    ...
    try {
      ...
      const cancel = service.cancelPairingFlow?.({ flowId: begin.flowId });
      const register = service.registerDevice({
      ...
      await expect(register).resolves.toMatchObject({ device_id: "device-2" });
  ...
  test("uses original flow device when approving and polling after later registration", async () => {
    const db = new Database(":memory:");
    db.exec(`
      CREATE TABLE sync_device_config (
        device_id TEXT PRIMARY KEY NOT NULL,
    ...
    secretService.entries.set("sync_device_id", "device-1");
    const snapshotDeviceIds: string[] = [];
    const service = createLocalDeviceSyncService({
      db,
      secretService,
      env: { CONNECT_API_URL: "https://api.example.test/" },
      ...
      fetch: async (input, init) => {
        ...
        if (url.endsWith("/api/v1/sync/snapshots/latest")) {
          snapshotDeviceIds.push(headers.get("x-wf-device-id") ?? "");
    ...
    try {
      ...
      await expect(service.approvePairingOverwrite?.({ flowId: begin.flowId })).resolves.toEqual({
        flowId: begin.flowId,
        phase: { phase: "syncing", detail: "waiting_snapshot" },
      });
      await expect(service.getPairingFlowState?.({ flowId: begin.flowId })).resolves.toEqual({
        flowId: begin.flowId,
        phase: { phase: "syncing", detail: "waiting_snapshot" },
      });
      expect(snapshotDeviceIds).toEqual(["device-1", "device-1"]);
      expect(JSON.parse(secretService.entries.get("sync_identity") ?? "{}")).toMatchObject({
        deviceNonce: "new-nonce",
        deviceId: "device-2",
      });
  ...
  test("runs issuer pairing operations through cloud when device id is configured", async () => {
    ...
    await expect(malformedSuccessService.approvePairing?.("pairing-1")).rejects.toMatchObject({
      code: "internal_error",
      message: "Failed to parse success response",
      status: 500,
    });
    await expect(malformedSuccessService.cancelPairing?.("pairing-1")).rejects.toMatchObject({
      code: "internal_error",
      message: "Failed to parse success response",
      status: 500,
    });
